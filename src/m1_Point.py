...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_taveled = self.distance_taveled + ((self.x - x)**2+(self.y - y)**2)**0.5
        self.x = x
        self.y = y
    ...
    def move_by(self, x, y):
        ...
        self.y = self.y + y
        self.count = self.count + 1
        self.distance_taveled = self.distance_taveled + ((x)**2+(y)**2)**0.5
    ...
    def get_distance_from_start(self):
        distance = ((self.x - self.initial_x) ** 2 + (self.y - self.initial_y) ** 2) ** 0.5
        return distance

    def get_distance_traveled(self):
        return self.distance_taveled

    def closer_to(self, alpha, beta):
        distance_to_alpha = ((self.x - alpha.x)**2+(self.y - alpha.y)**2)**0.5
        distance_to_beta = ((self.x - beta.x)**2+(self.y - beta.y)**2)**0.5
        if distance_to_alpha <= distance_to_beta:
            return alpha
        else:
            return beta

    def halfway_to(self, point):
        return 'Point({}, {})'.format((self.x + point.x)/2, (self.y + point.y)/2)
